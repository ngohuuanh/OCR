...
class AuthService {
  ...
  // api create user on db
  _createUser(UserCredential authResult, String name) async {
    await post(
      'http://10.0.2.2:4000/user/add',
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode({'uid': '${authResult.user.uid}', 'name': '$name'}),
    );
  }

//API check user exist ?
  _checkUser(UserCredential authResult) async {
    Response response = await get(
      'http://10.0.2.2:4000/user/check/ex/${authResult.user.uid}',
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
    );
    ...
    if (response.statusCode == 200) {
      return jsonDecode(response.body)['rowCount'];
    } else {
  ...
  Future<String> createUserWithEmailAndPassword(
    ...
    //api create user
    await _createUser(authResult, name);
    // Update the username
  ...
  Future<String> signInWithEmailAndPassword(
    ...
    final _checkRow = await _checkUser(authResult);
  ...
  //Delete user current not for admin
